...
If you don't provide the `value` prop for `<Progress>`, then the progress bar will be _indeterminate_.
This is useful for showing that some progress is going on, but the actual duration is not yet determined.
